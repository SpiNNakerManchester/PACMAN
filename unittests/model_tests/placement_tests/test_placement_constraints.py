...
class TestPlacementConstraints(unittest.TestCase):
    ...
    def test_board_constraint(self):
        ...
        self.assertEquals(c1.board_address, "1.2.3.4")
        self.assertEquals(c1, BoardConstraint("1.2.3.4"))
        self.assertEquals(str(c1), 'BoardConstraint(board_address="1.2.3.4")')
    ...
    def test_chip_and_core_constraint(self):
        ...
        self.assertEquals(c1.x, 1)
        self.assertEquals(c1.y, 2)
        self.assertEquals(c1.p, None)
        self.assertEquals(c1.location, {"x": 1, "y": 2, "p": None})
        self.assertEquals(c1, ChipAndCoreConstraint(1, 2))
        self.assertEquals(str(c1), 'ChipAndCoreConstraint(x=1, y=2, p=None)')
    ...
    def test_radial_placement_from_chip_constraint(self):
        ...
        self.assertEquals(c1.x, 1)
        self.assertEquals(c1.y, 2)
        self.assertEquals(c1, RadialPlacementFromChipConstraint(1, 2))
        self.assertEquals(str(c1),
    ...
    def test_same_chip_as_constraint(self):
        ...
        self.assertEquals(c1.vertex, v1)
        self.assertEquals(str(c1), "SameChipAsConstraint(vertex=v1)")
        self.assertEquals(str(c4), "SameChipAsConstraint(vertex=v2)")
