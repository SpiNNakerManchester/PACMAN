...
class FixedRouteRouter(object):
    ...
    def _do_dynamic_routing(
        ...
        for (chip_x, chip_y) in routing_tables_by_partition.get_routers():
            ...
            key = machine.get_global_xy(chip_x, chip_y, eth_x, eth_y)
    ...
    def _do_fixed_routing(
        ...
        for path_id in paths:
            ...
            for (path_chip_x, path_chip_y) in paths[path_id]:
                ...
                chip_xy = machine.get_global_xy(
                    path_chip_x, path_chip_y, ethernet_chip_x, ethernet_chip_y)
                fixed_route_tables[chip_xy] = fixed_route_entry
