...
class AbstractPlacerAlgorithm(object):
    ...
    def _reduce_constraints(self, constraints, subvertex_label, placements):
        """ Try to reduce the placement constraints into one constraint.
        ...
        :type constraints: iterable of\
                    :py:class:`pacman.model.constraints.AbstractConstraints`
        ...
        :rtype:\
                    :py:class:`pacman.model.constraints.PlacerChipAndCoreConstraint`
        ...
        for constraint in constraints:
            if isinstance(constraint, PlacerChipAndCoreConstraint):
                ...
                if other_placement is not None:
                    x = self._check_param(x, other_placement.x,
                                          subvertex_label)
                    y = self._check_param(y, other_placement.y,
                                          subvertex_label)
                    p = self._check_param(p, other_placement.p,
                                          subvertex_label)
    ...
    def _check_param(param_to_check, param_to_update, subvertex_label):
        """ Check if constraints with overlapping requirements are satisfiable
        ...
        :rtype: int
        :raise PacmanPlaceException: When the settings of the parameters are\
                    not compatible
