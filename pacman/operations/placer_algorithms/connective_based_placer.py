...
class ConnectiveBasedPlacer(RadialPlacer):
    """ A radial algorithm that can place a partitioned_graph onto a
        ...
        and which will place highly connected cores on the same chip or close
        by
    """
    ...
    def place(self, partitioned_graph):
        ...
        # loop around locating next most connected to what's already placed
        # so that eventually there's none left
        while len(unconstrained_vertices) > 0:
            next_subvert = self._locate_next_most_connected(
                unconstrained_vertices, completed)
    ...
    def _sort_out_unconstrained_vertices_on_connectivity(
        ...
        :param unconstrained_vertices: the list of unordered unconstrained\
                    vertices
