...
class VertexBasedRoutingInfoAllocator(object):
    ...
    def __call__(self, partitionable_graph, graph_mapper, subgraph, n_keys_map,
        ...
        :return: routing information objects and the routing tables for the\
                spinnaker machine
        """
        ...
        progress_bar.update(len(subgraph.partitions))
    ...
    def _adds_application_field_to_the_fields(self, seen_fields):
        """ Determine what the field spaces and values are for the\
            ...
            fields and adds it to the fields
        ...
        if field_utilties.TYPES_OF_FIELDS.FIXED_KEY.name in seen_fields:
            success = self._fixed_key_application_field_allocation(
                seen_fields, required_bits)
        else:
            if field_utilties.TYPES_OF_FIELDS.FIXED_MASK.name in seen_fields:
    ...
    def _fixed_key_application_field_allocation(
        ...
        :return:
        """
        fixed_keys = seen_fields[field_utilties.TYPES_OF_FIELDS.FIXED_KEY.name]
        if len(fixed_keys) == 1:
            found = False
            searching = True
            ...
            fields = field_utilties.convert_mask_into_fields(fixed_key.mask)
            ...
            while searching:
                ...
                seen_fields[field_utilties.TYPES_OF_FIELDS.FIXED_KEY.name] \
                    = list()
                seen_fields[field_utilties.TYPES_OF_FIELDS.FIXED_KEY.name]\
                    .append((fixed_key, new_fields))

                # if there's fixed masks as well, ensure they work with the
                field_name = field_utilties.TYPES_OF_FIELDS.FIXED_MASK.name
            ...
            while searching:
                ...
                for fixed_key_index in range(1, len(fixed_keys)):
                    ...
                    fields = \
                        field_utilties.convert_mask_into_fields(fixed_key.mask)
                ...
                if valid:
                    ...
                    for fixed_key_index in range(1, len(fixed_keys)):
                        fixed_key = fixed_keys[fixed_key_index]
                        fields = field_utilties.convert_mask_into_fields(
                            fixed_key.mask)
                        # create new fields
                        new_fields = self.\
                    ...
                    if field_name in seen_fields:
                        ...
                        if success:
                            ...
                            seen_fields[field_utilties.TYPES_OF_FIELDS.
    ...
    def _fixed_mask_application_field_allocation(
        ...
        """
        fixed_mask_masks = \
            seen_fields[field_utilties.TYPES_OF_FIELDS.FIXED_MASK.name]
    ...
    def _update_fixed_mask_field_set(self, bit_generator, fixed_key=None):
        """ Updates the first mask's field set so that it has a application\
    ...
    def _adjust_fixed_mask_fields_for_application_field(
        ...
        fields = field_utilties.convert_mask_into_fields(new_mask)
        # re-adjust the fields to link to the original mask (needed for mapping
        # later on)
        for field in fields:
    ...
    def _extract_keys_and_masks_from_bit_field(
        ...
        """ Takes the bit field space and a partition and locates the keys and\
        ...
        if len(fixed_key_constraints) > 0:
            ...
            for fixed_key_field in fixed_keys_fields:
                ...
                if tag == SUPPORTED_TAGS.ROUTING.name:
                    ...
                    raise exceptions.PacmanConfigurationException(
                        "Don't know this tag field")

            if len(range_based_fixed_key_fields) > 1:
                raise exceptions.PacmanConfigurationException(
                    "This algorithm is not designed to work with multiple"
                    " ranged based fields for a fixed key.")
            ...
            if len(range_based_fixed_mask_fields) > 1:
                raise exceptions.PacmanConfigurationException(
                    "This algorithm is not designed to work with multiple"
                    " ranged based fields for a fixed mask.")
            ...
            raise exceptions.PacmanConfigurationException(
                "Not yet implemented fixed field constraints")
            ...
            # if the set contains a ranfe_based flexible field do search
    ...
    def _check_keys_are_continious(keys_and_masks):
        """ Search though the keys and masks and checks if they are all\
            continuous
    ...
    def _handle_set_of_flexi_range_fields(
            ...
            position):
        """ Takes a set of flexible fields which are range based and deduces\
    ...
    def _create_application_space_in_the_bit_field_space(
        ...
        for field in fields:
            ...
            if field == field_utilties.TYPES_OF_FIELDS.FIXED_MASK.name:
                fixed_fields = \
                    fields[field_utilties.TYPES_OF_FIELDS.FIXED_MASK.name]
            ...
            # handle fixed field
            elif field == field_utilties.TYPES_OF_FIELDS.FIXED_FIELD.name:

                # TODO need to check this bit out
                raise exceptions.PacmanConfigurationException(
                    "Fixed fields are not yet supported")

            # handle fixed key fields
            elif field == field_utilties.TYPES_OF_FIELDS.FIXED_KEY.name:
                fixed_keys = \
                    fields[field_utilties.TYPES_OF_FIELDS.FIXED_KEY.name]
    ...
    def _locate_application_field(self, fields):
        ...
        when its found, deduces what's left for space space for other fields
        ...
        for field in fields:
            ...
            if field == field_utilties.TYPES_OF_FIELDS.FIXED_MASK.name:
                ...
                for (_, fixed_key_fields) in fixed_keys_fields:
                    for fixed_key_field in fixed_key_fields:
                        if (fixed_key_field.name ==
                                APPLICATION_DIVIDER_FIELD_NAME):
                            if application_feild is None:
                                application_feild = fixed_key_field
                                application_field_spare_values = \
                                    self._deduce_application_field_space(
                                        application_feild)
                            else:
                ...
                raise exceptions.PacmanConfigurationException(
                    "Fixed fields have not yet been implemented")
    ...
    def _create_flexi_field_space_in_the_bit_field_space(
        ...
        """ Creates the fields in the bit field space for the flexible fields
    ...
    def _deduce_application_field_space(application_field):
        """ Given an application field, deduces the spare spaces available\
            to it
    ...
    @staticmethod
    def _locate_field_value_by_name(feild_list, field_name):
        """
