...
class _Merge(object):
    ...
    def apply(self, aliases: Dict[Tuple[int, int], Set[Tuple[int, int]]]
              ) -> Tuple[List[RTEntry],
        ...
        our_aliases: Set[Tuple[int, int]] = set()
        aliases[self.key, self.mask] = our_aliases
        ...
        return [ent for ent in new_table if ent is not None], aliases


def minimise(
        routing_table: List[RTEntry], use_timer_cut_off: bool = False,
        time_to_run_for_before_raising_exception: Optional[int] = None):
    """
...
def ordered_covering(
        ...
        aliases: Optional[Dict[Tuple[int, int], Set[Tuple[int, int]]]] = None,
        no_raise: bool = False, use_timer_cut_off: bool = False,
        time_to_run_for: Optional[int] = None) -> Tuple[
            List[RTEntry], Dict[Tuple[int, int], Set[Tuple[int, int]]]]:
    """
    Reduce the size of a routing table by merging together entries where
    possible.
    ...
    :raises MinimisationFailedError:
        ...
        ``target_length``.
    """
    # Copy the _aliases dictionary, handle default
    _aliases = dict(aliases) if aliases is not None else {}
    ...
    while target_length is None or len(routing_table) > target_length:
        # Get the best merge
        merge = _get_best_merge(routing_table, _aliases)
        ...
        # _aliases dictionary.
        routing_table, _aliases = merge.apply(_aliases)
    ...
    # Return the finished routing table and _aliases table
    return routing_table, _aliases
...
def _get_best_merge(
        routing_table: List[RTEntry],
        aliases: Dict[Tuple[int, int], Set[Tuple[int, int]]]) -> _Merge:
    """
...
def _refine_merge(
        merge: _Merge, aliases: Dict[Tuple[int, int], Set[Tuple[int, int]]],
        min_goodness: int) -> _Merge:
    """
...
def _get_covered_keys_and_masks(
        merge: _Merge,
        aliases: Dict[Tuple[int, int], Set[Tuple[int, int]]]) -> Iterable[
            Tuple[int, int]]:
    """
