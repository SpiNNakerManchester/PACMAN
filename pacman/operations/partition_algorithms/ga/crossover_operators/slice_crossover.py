...
class GaSliceCrossoverKPoints(AbstractGaCrossover):
    ...
    def _do_crossover(self, individual1: GASliceSolutionRepresentation, individual2: GASliceSolutionRepresentation) -> Tuple[AbstractGaCrossover, AbstractGaCrossover]:
        ...
        while P[0] < individual1_length and P[1] < individual2_length:
            select_individual = 0
            if len(new_individual1[0]) == 0 or (individuals[0][P[select_individual]][self.SLICE_RECORD_NEURON_TO_INDEX] > new_individual1[0][-1] and individuals[1][P[select_individual]][self.SLICE_RECORD_NEURON_TO_INDEX] > new_individual1[0][-1]):
                select_individual = random.choices([0, 1], k = 1)[0]
            else:
                if individuals[0][P[select_individual]][self.SLICE_RECORD_NEURON_TO_INDEX] < new_individual1[0][-1]:
                    select_individual = 1
                elif individuals[1][P[select_individual]][self.SLICE_RECORD_NEURON_TO_INDEX] < new_individual1[0][-1]:
                    select_individual = 0
                else:
                    raise ValueError
            ...
            while P[1 - select_individual] < individual2_length and \
                len(new_individuals[1 - select_individual][0]) == 0 or (new_individuals[1 - select_individual][0][-1] < new_individuals[select_individual][0][-1]):
