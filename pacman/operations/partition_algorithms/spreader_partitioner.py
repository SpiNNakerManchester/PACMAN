...
class SpreaderPartitioner(object):
    """ Partitioner that tries to fill the space available
    """
    ...
    def __call__(self, graph, machine):
        ...
        # Load the machine and vertices objects
    ...
    def _partition_vertices_on_specific_core(
            ...
            done_vertices, resource_allocation):
        """ Partitions vertices that need to go on a specific core
        """
        ...
        for vertex in core_constrained_vertices:
            max_atom_constraint = utility_calls.locate_constraints_of_type(
            ...
            if vertex.atoms > max_atoms:
                ...
                subvertex = vertex.create_subvertex(
                    ...
                    label=(
                        "partitioned vertex with atoms {} to {} for vertex {}"
                        .format(
        ...
        for vertex in chip_constrained_vertices.keys():
            if vertex not in done_vertices:
                ...
                total_free_processors_on_chip = \
                    resource_allocation.total_free_processors_on_chip(
                ...
                min_per_core = math.ceil(
                    total_atoms / total_free_processors_on_chip)
    ...
    def _locate_placement_constrained_vertices(vertices):
        """ Locate constrained vertices

        :param vertices: the partitionable graphs vertices
        :return: 4 lists where the first is vertices which are constrained to\
                    a processor and the second is constrained to a chip,\
                    third is ones which need to be placed on the same chip as\
                    others and last is not constrained
        """
    ...
    def _partition_none_constrained(
        ...
        min_per_core = int(math.ceil(
            float(total_atoms) / float(free_processors)))
        if min_per_core == 1:
            min_per_core = 0
    ...
    def _determine_min_max_atoms(self, vertices, done_vertices):
        """ Determine what the minimum number of atoms respecting the maximum\
            of the vertices

        :param vertices: the vertices of the application
        :return: a minimum maximum number of atoms
        """
        min_max = sys.maxint
        for vertex in vertices:
            if vertex not in done_vertices:
