...
class overrides(object):
    ...
    def __call__(self, method):
        ...
        self.__sanity_check_name(method)
        ...
        if method.__name__ != "__init__":
            self.__sanity_check_arguments(method)

        # Copy the doc string if necessary/requested
        self.__copy_documentation_strings(method)

        return method

    def __sanity_check_name(self, method):
        if method.__name__ != self._superclass_method.__name__:
            raise AttributeError(
                "Superclass method name {} does not match {}. Ensure override "
                "is the last decorator before the method declaration".format(
                    self._superclass_method.__name__, method.__name__))

    def __sanity_check_arguments(self, method):
        ...
        if len(all_args) != len(super_args.args):
            raise AttributeError(
                "Method has {} arguments but superclass method has {} "
                "arguments".format(
                    len(method_args.args), len(super_args.args)))

        for arg, super_arg in zip(all_args, super_args.args):
            if arg != super_arg:
                raise AttributeError("Missing argument {}".format(super_arg))

        if ((default_args is None and super_args.defaults is not None) or
                (default_args is not None and super_args.defaults is None)
                or (default_args is not None
                    and super_args.defaults is not None
                    and len(default_args) != len(super_args.defaults))):
            raise AttributeError(
                "Default arguments don't match superclass method")

    def __copy_documentation_strings(self, method):
        superdoc = self._superclass_method.__doc__
        if superdoc is not None:
            if method.__doc__ is None:
                method.__doc__ = superdoc
            elif self._extend_doc:
                method.__doc__ = superdoc + method.__doc__
