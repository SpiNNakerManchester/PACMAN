...
class SplitterOneToOneLegacy(AbstractSplitterCommon[ApplicationVertex]):
    ...
    def set_governed_app_vertex(self, app_vertex: ApplicationVertex):
        ...
        self._vertex_slice = Slice(0, app_vertex.n_atoms - 1)
        self._sdram = app_vertex.get_sdram_used_by_atoms(self._vertex_slice)
        self._machine_vertex = app_vertex.create_machine_vertex(
            vertex_slice=self._vertex_slice, sdram=self._sdram, label=None)
        app_vertex.remember_machine_vertex(self._machine_vertex)
